...
class UnifiedRolloutManager:
    ...
    def collect(self, policy, steps: int = 128) -> Tuple[List[Dict[str, Any]], List[Dict]]:
        ...
        for _ in range(steps):
            ...
            for i in gameplay_env_indices + trade_env_indices:
                ...
                # Collect gameplay need events for ToM supervision
                if phase_name == "gameplay" and env_action.get("type") in ["build_settlement", "build_city", "build_road", "buy_dev_card"]:
                    need_event = self._extract_gameplay_need_event(env_action)
                    if need_event:
                        gameplay_need_events.append(need_event)
